...
mko_clean |> 
  ...
  ggplot(aes(x = Temp_bot)) +
  geom_density(alpha = 0.5, fill = "gray30") +
  facet_wrap(~month_name) +
  theme_bw() 
```
...
ggplot(data = mko_clean, aes(x = Temp_bot, y = month_name, fill = after_stat(x))) +
...
ggplot(data = mko_clean, aes(x = month_name, y = Temp_bot)) +
  geom_boxplot() +
