...
ggplot(data = penguin_summary, aes(x = species, y = n)) +
...
#| warning: false
ggplot(data = penguins, 
       mapping = aes(x = species, y = after_stat(prop), group = 1)) + 
  geom_bar() +
  theme_bw()
```
...
```{r}
#| warning: false
ggplot(data = penguins, aes(x = species, fill = island)) +
  geom_bar() +
  theme_bw()
```
...
ggplot(data = penguins, aes(x = flipper_length_mm, fill = species)) +
  ...
  y = "Frequency", fill = "Species",
  title = "Penguin Fipper Lengths") +
